...
class DemoApplicationTests(
    ...
    fun `test save person data`() {
        ...
        repository.findAll().filter { it.firstname == person.firstname }
            .count()
    ...
    fun `test findAll person data`() {
        ...
        repository.saveAll(personList).blockLast()
    ...
    fun `test findById`() {
        ...
        client.get().uri("/persons/${savedPerson.id}")
            .exchange()
            .expectStatus().isOk
            .expectHeader().contentType(MediaType.APPLICATION_JSON)
    ...
    fun `test findById if entity not exist`() {
        client.get().uri("/persons/${Random.nextLong()}")
            .exchange()
    ...
    fun `test update if entity not exist`() {
        val person = generateRandomPerson()
        client.put().uri("/persons/${Random.nextLong()}")
            .bodyValue(person)
            .exchange()
            .expectStatus().isBadRequest
    }
    ...
    fun `test delete if entity not exist`() {
        client.delete().uri("/persons/${Random.nextLong()}")
            .exchange()
            .expectStatus().isBadRequest
    }
